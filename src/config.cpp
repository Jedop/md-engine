...
void print_help() {
    std::cout << "Usage: ./MDEngine [options]\n\n"
              << "--dt <value>\n"
              << "--frames <value>\n"
              << "--rho <value>\n"
              << "--fcc / --sc\n"
...
void print_config(const SimConfig& cfg) {
    ...
    std::cout << "frames=" << cfg.frames << ", dt=" << cfg.dt << ", rho=" << cfg.rho
              << ", lattice=" << (cfg.fcc_or_not ? "FCC" : "SC")
              << ", particles_per_side=" << cfg.particles_per_side
              << ", unit_cells=" << cfg.unit_cells_per_side
              << ", thermostat=" << cfg.use_thermostat << ", target_T=" << cfg.target_T
              << ", annealing=" << cfg.do_annealing << ", time_reversal=" << cfg.do_time_reversal
              << ", traj=" << cfg.traj_file << ", data=" << cfg.data_file << ", gpu=" << cfg.gpu
              << ", eq-steps=" << cfg.eq_steps << "\n\n";
}
...
void parse_args(int argc, char** argv, SimConfig& cfg) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "-h" || arg == "--help") {
            ...
            if (i + 1 >= argc) {
                std::cerr << "--thermostat requires 0 or 1\n";
            }

            cfg.use_thermostat = std::stoi(argv[++i]) != 0;
        }
            ...
            if (backend == "gpu")
                ...
                std::cerr << "Error: invalid backend '" << backend << "'. Use 'cpu' or 'gpu'.\n";
                exit(1);
            }
        ...
        else if (arg == "--eq-steps" && i + 1 < argc)
            cfg.eq_steps = std::stoi(argv[++i]);
    }
