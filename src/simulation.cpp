#include "simulation.hpp"
#include "utils.hpp"

std::tuple<double, double, double> update(std::vector<Particle>& Particles, std::vector<int>& head,
                                          std::vector<int>& next, double dt, int nx,
                                          double cell_size, double box, GpuMemory& mem,
                                          bool use_gpu) {
    double potential_energy;
    double kinetic_energy = 0;

    if (use_gpu) {
        std::tie(potential_energy, kinetic_energy) =
            compute_all_forces_gpu(Particles, box, mem, dt);
    } else {
        VelocityVerlet::step1(Particles, dt);

        std::vector<Vec3> new_acc;

        build_cell_lists(Particles, head, next, nx, cell_size);
        std::tie(new_acc, potential_energy) =
            compute_all_forces(Particles, head, next, nx, cell_size, box);

        // 4. Post-Force Update (Finish the time step using the new forces)
        VelocityVerlet::step2(Particles, new_acc, dt);

        for (const auto& p : Particles) {
            Vec3 v = p.velocity;
            // v = p.velocity - p.acceleration * dt * 0.5; Leapfrog Calculation
            double v2 = v.x * v.x + v.y * v.y + v.z * v.z;
            kinetic_energy += 0.5 * v2;
        }
    }

    double Energy = potential_energy + kinetic_energy;

    return {potential_energy, kinetic_energy, Energy};
}

void run_simulation(SimConfig config) {
    auto start = std::chrono::steady_clock::now();
    if (config.use_thermostat && config.do_time_reversal) {
        std::cout << "Don't try to use thermostat and do time reversal together, do "
                     "either one. For now, it'll act as if the thermostat is off.";
    }

    // Constants
    const int N = config.fcc_or_not ? 4 * std::pow(config.unit_cells_per_side, 3)
                                    : std::pow(config.particles_per_side, 3);
    const double box = pow(N / config.rho, 1.0 / 3.0);
    const int nx = int(box / rc);
    const int num_cells = nx * nx * nx;
    const double cell_size = box / nx;

    // Initialization
    std::vector<Particle> Particles;
    std::vector<Vec3> initial_acc;
    double dummy_pot;

    if (config.fcc_or_not) {
        Particles = init_fcc_lattice(config.unit_cells_per_side, box);
    } else {
        Particles = init_sc_lattice(config.particles_per_side, box);
    }

    // This function also adds random velocities but I didn't know how to fit all
    // that information into one function name
    remove_center_of_mass_momentum(Particles);

    // Creates a Structure of Pointers, which (if config.gpu == 1) points to first element of
    // various arrays/doubles on the GPU
    GpuMemory gpu_mem;
    if (config.gpu) {
        gpu_mem = allocate_gpu_memory(Particles.size(), num_cells);
        init_gpu_memory(Particles, gpu_mem);
    }

    // head and next vectors act as linked list (unused if using gpu)
    std::vector<int> head(num_cells, -1);
    std::vector<int> next(Particles.size());

    // Files into which everything is written
    std::ofstream traj_file(config.traj_file);
    std::ofstream data_file(config.data_file);

    // Initial build of cell list and other initial stuff(Unused if using GPU, simply calculates
    // forces)
    if (config.gpu) {
        init_gpu_physics(gpu_mem, N, box, 1 / box, nx, cell_size);
    } else {
        build_cell_lists(Particles, head, next, nx, cell_size);
        std::tie(initial_acc, dummy_pot) =
            compute_all_forces(Particles, head, next, nx, cell_size, box);
        for (size_t i = 0; i < Particles.size(); i++) {

            Particles[i].acceleration = initial_acc[i];
            // Leapfrog Initialization
            // Particles[i].velocity += Particles[i].acceleration * dt * 0.5;
        }
    }

    // Runs if you want some equilibrating steps
    if (config.eq_steps > 0) {
        std::cout << "\n--- Starting Equilibration Phase ---\n";
        auto eq_start = std::chrono::steady_clock::now();

        for (int step = 1; step <= config.eq_steps; step++) {
            auto [pot_E, kin_E, Tot_E] =
                update(Particles, head, next, config.dt, nx, cell_size, box, gpu_mem, config.gpu);

            if (config.use_thermostat && !config.do_time_reversal) {
                double T_current = (2 * kin_E) / (3 * Particles.size());
                if (config.gpu) {
                    apply_berendsen_thermostat_gpu(Particles, T_current, config.target_T, config.dt,
                                                   gpu_mem);
                } else {
                    apply_berendsen_thermostat(Particles, T_current, config.target_T, config.dt);
                }
            }

            if (step % 100 == 0 || step == config.eq_steps) {
                auto now = std::chrono::steady_clock::now();
                double elapsed = std::chrono::duration<double>(now - eq_start).count();
                print_progress(step, config.eq_steps, elapsed);
            }
        }
    }

    // Recording initial positions in case of time reversal
    std::vector<Vec3> initial_positions(Particles.size());
    for (size_t i = 0; i < Particles.size(); i++) {
        initial_positions[i] = Particles[i].position;
    }

    // Main Loop
    for (int step = 0; step < config.frames; step++) {

        // --- FEATURE: TIME REVERSAL ---
        if (config.do_time_reversal && step == (config.frames / 2)) {
            if (config.gpu) {
                reverse_velocities_gpu(N, gpu_mem);
            } else {
                for (auto& p : Particles)
                    p.velocity = p.velocity * -1.0;
            }
            std::cout << ">>> TIME REVERSAL TRIGGERED <<<\n";
        }

        // --- FEATURE: ANNEALING ---
        if (config.do_annealing) {
            if (step < config.frames * 0.75) { // Cool down over 75% of the run
                config.target_T = 1.5 - (1.4 * ((double)step / (config.frames * 0.75)));
            } else {
                config.target_T = 0.1;
            }
        }

        // --- CORE PHYSICS ---
        auto [pot_E, kin_E, Tot_E] =
            update(Particles, head, next, config.dt, nx, cell_size, box, gpu_mem, config.gpu);

        // --- FEATURE: THERMOSTAT ---
        if (config.use_thermostat && !config.do_time_reversal) {
            // Safety: Never use thermostat during time reversal!
            double T_current = (2 * kin_E) / (3 * Particles.size());
            if (config.gpu) {
                apply_berendsen_thermostat_gpu(Particles, T_current, config.target_T, config.dt,
                                               gpu_mem);
            } else {
                apply_berendsen_thermostat(Particles, T_current, config.target_T, config.dt);
            }
        }

        if (step % 100 == 0 && step > 0) {
            if (config.gpu) {
                sync_gpu_to_cpu(Particles, gpu_mem);
            };
            auto now = std::chrono::steady_clock::now();
            double elapsed = std::chrono::duration<double>(now - start).count();

            print_progress(step, config.frames, elapsed);

            traj_file << Particles.size() << "\n";

            traj_file << "Lattice=\"" << box << " 0.0 0.0 0.0 " << box << " 0.0 0.0 0.0 " << box
                      << "\" Properties=species:S:1:pos:R:3\n";

            for (const auto& p : Particles) {
                traj_file << "Ar " << p.position.x << " " << p.position.y << " " << p.position.z
                          << "\n";
            }

            data_file << pot_E << " " << kin_E << " " << Tot_E << " " << (2 * kin_E / (3 * N))
                      << "\n";
        }
    }
    traj_file.close();
    data_file.close();

    auto now = std::chrono::steady_clock::now();
    double elapsed = std::chrono::duration<double>(now - start).count();

    print_progress(config.frames, config.frames, elapsed);
    std::cout << "\n";

    if (config.gpu) {
        sync_gpu_to_cpu(Particles, gpu_mem);
        free_gpu_memory(gpu_mem);
    }

    // Output Time Reversal Error
    if (config.do_time_reversal) {
        double max_error = 0.0;

        for (size_t i = 0; i < Particles.size(); i++) {
            Vec3 diff = Particles[i].position - initial_positions[i];

            // PBC is unnecessary because ideally they'll be at initial positions
            double error_distance = std::sqrt(diff.x * diff.x + diff.y * diff.y + diff.z * diff.z);

            if (error_distance > max_error) {
                max_error = error_distance;
            }
        }

        std::cout << "====================================\n";
        std::cout << "Time Reversibility Maximum Absolute Error: " << std::scientific << std::setprecision(6) << max_error << "\n";
        std::cout << "====================================\n";
    }
}
